...
기존 체크리스트 위에 얹는 5개. 문장이 느슨할 때 먼저 검색.

- **구체성**: 추상을 숫자·감각으로. "자주 간다" → "한 달에 여섯 번 간다". "키가 크다" → "184센티미터이다".
- **"있다" 지우기**: `~하고 있다` → `~한다`. "싸우고 있다" → "싸운다". 상태 지속이 핵심일 때만 예외 ("문이 열려 있다").
- **"것" 지우기**: 명사화 과잉. "오고 있었던 것이다" → "오던 길이었다". "중요한 것은 속도다" → "속도가 중요하다".
- **"수" 지우기**: `~할 수 있다` 남발은 can 직역. "걸릴 수도 있다" → "걸릴지도 모른다" / "걸리기도 한다". 능력이 핵심일 때만 유지.
- **접속사 지우기**: `그리고 / 그래서 / 하지만 / 그러므로`. 문단 첫 문장이 접속사로 시작하면 거의 삭제 가능.
